...
def get_assigned_group(user_location,company_division):
	print("user location ................ {}".format(user_location))
	group = {}
	with open("/var/www/html/bmc_assigned_group.json") as f:
		jData = json.load(f)
	 
	for key,value in jData.items():
		# if user_location.lower() in key:
		if key in user_location.lower():
			print("PLEASE TELL ME THE GROUP VALUE EVERYTIME...............{}".format(value))
			group = value
			break
...
def push_ticket_details_mongo(sessionId,**kwargs):
	...
	if ticket_details_row:
		...
		#print "Pura details delo...{}".format(sessionId)
...
def getResponse(input_statement,sessionId,response,cont_id):
	...
	Corporate_ID = Summary = Urgency = Impact = Assigned_Group = MNM_Chatbot_ID = Notes = user_role_band = ticket_type = ""
	...
	if chat_stage == "sop":
		...
		if len(input_statement.split()) < 2:
			...
			replyJson.update({"session_ended":session_ended})
			convoTime = datetime.datetime.now()
			print("JUST BEFORE================{}".format(Notes))
			...
			replyJson.update({"session_ended":session_ended})
			convoTime = datetime.datetime.now()
			print("UNIDENTIFIED JUST BEFORE================{}".format(Notes))
			push_ticket_details_mongo(sessionId,Notes=Notes)
			pushLastResponseInMongo(sessionId,replyVal,category,sub_bucket,input_statement,count_reply,mongo_bucket,note_flag,chat_stage,ticket_number,session_ended, ticket_type)
			pushConversationLogsToMongo(sessionId,convoTime,input_statement,replyVal,user_name,cont_id,Notes)
			return replyJson
		...
		if len(input_statement.split()) < 2:
			...
			replyJson.update({"session_ended":session_ended})
			convoTime = datetime.datetime.now()
			print("SR JUST BEFORE================{}".format(Notes))
			push_ticket_details_mongo(sessionId,Notes=Notes)
			pushLastResponseInMongo(sessionId,replyVal,category,sub_bucket,input_statement,count_reply,mongo_bucket,note_flag,chat_stage,ticket_number,session_ended, ticket_type)
			pushConversationLogsToMongo(sessionId,convoTime,input_statement,replyVal,user_name,cont_id,Notes)
			return replyJson	
			...
			resp = requests.request("get","https://emss.mahindra.com/sap/bc/zhr_tq_emp_det?sap-client=500&IV_PERNR="+Corporate_ID)
			resp_list = resp.text.split("-//-")
			#print "Service REQUEST....{}".format(resp_list)
			try:
				mobile = resp_list[18]
				pushUserDetailsToMongo(sessionId, mobile=mobile)
			except:
		...
		if len(input_statement.split()) < 2:
			...
			replyJson.update({"session_ended":session_ended})
			convoTime = datetime.datetime.now()
			print("INCIDENT JUST BEFORE============{}".format(Notes))
			push_ticket_details_mongo(sessionId,Notes=Notes)
			pushLastResponseInMongo(sessionId,replyVal,category,sub_bucket,input_statement,count_reply,mongo_bucket,note_flag,chat_stage,ticket_number,session_ended, ticket_type)
			pushConversationLogsToMongo(sessionId,convoTime,input_statement,replyVal,user_name,cont_id,Notes)
			return replyJson
			...
			replyJson.update({"session_ended":session_ended})
			convoTime = datetime.datetime.now()
			print("MOBILE JUST BEFORE============={}".format(Notes))
			push_ticket_details_mongo(sessionId,Notes=Notes)
			pushLastResponseInMongo(sessionId,replyVal,category,sub_bucket,input_statement,count_reply,mongo_bucket,note_flag,chat_stage,ticket_number,session_ended, ticket_type)
			pushConversationLogsToMongo(sessionId,convoTime,input_statement,replyVal,user_name,cont_id,Notes)
			return replyJson
		...
		print("gctdfhuguiyivbm----------{}".format(cont_id))
		...
		if input_statement.lower() == "skip":
			...
			assigned_group = get_assigned_group(location,company_division)
	...
	if chat_stage == "mobile_incidentreq":
		...
		print("Mobile number batao bhai.........{}".format(mobile))
		if input_statement.lower() == "skip":
	...
	if chat_stage == "2" or chat_stage == "ticket_confirmation":
		...
		if input_statement == "cancel":
			...
			if ticket_type == "incident" or ticket_type == "unidentified":
				try:
					...
					print("BMC SERVICE GOT CALLED............{}".format(mobile))
					ticket_details_row = ticket_details_collection.find_one({"sessionId":sessionId})
					if ticket_details_row:
						mongoObjjson = json.dumps(list(ticket_details_row),default=json_util.default)
				                mongoObjjson = json.loads(mongoObjjson)
                				mongoObjjson = filter(None,mongoObjjson)
                				if mongoObjjson:
                        				mobile = ticket_details_row['Mobile_Num']	
					print("Final mobile number............{}".format(mobile))
					ticket_number = create_ticket_bmc(token_number=Corporate_ID, mobile_number=mobile, issue=Summary, description=Notes, asset=asset, chatbot_unique_id=sessionId, Assigned_Group=Assigned_Group)
				except:
				...
				#assigned_gp = get_assigned_group(company_address,company_division)
				#push_to_ticket_archieve_mongo(sessionId, Assigned_Group=assigned_gp)
				print("Ticket Type.................{}".format(ticket_type))
				print("WORKORDER GOT CREATED............{}".format(mobile))
				ticket_number = create_ticket_work_order(token_number=Corporate_ID, issue=Summary, description=Notes, request_type_id=request_type_id, chatbot_unique_id=sessionId, Assigned_Group=Assigned_Group)
				#except:
				#	api_error_report = traceback.format_exc()
...
def application(environ, start_response):
	...
	if poll_flag == "true":
		...
		pushUserDetailsToMongo(id,user_name = user_name, company_name = company_name, company_address = company_address, company_division = company_division, cont_id = cont_id, role = role, reports_to = reports_to, user_role_band=user_role_band, chatbot_tag = "MSolve")
		reply = {"sessionId":id}
