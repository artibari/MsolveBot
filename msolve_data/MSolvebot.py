...
from msolve_soap_apis import get_asset_by_user
...
def getResponse(input_statement,sessionId,response,cont_id):
	...
	Corporate_ID = Summary = Urgency = Impact = Assigned_Group = MNM_Chatbot_ID = Notes = user_role_band = asset = asset_Empty = location_inp=""
	...
	if last_resp_row:
		...
		if mongoObjjson:
			...
			asset_Empty = mongoObjjson[0].get('asset_Empty')
	...
	if user_details_row:
		...
		if mongoObjjson:
			...
			asset_Empty = mongoObjjson[0].get('asset_Empty')
			print "USer Details..............{}".format(mongoObjjson[0])
	...
	if chat_stage == "1":
		if input_statement == "skip":
			...
			print("LOCATION DETAILS...................{}, {}".format(location_details, len(location_details)))
			if len(location_details) > 1:
				location_input = location_details[0].lower().strip()
				print("LOCATION INPUT.....................{}".format(location_input))
				company_division = location_details[1].strip()
				print("COMPANY LOCATION INPUT.....................{}".format(company_division))
			else:
				location_input = location_details[0].lower().strip()
			...
			if str(location_input) in location_list:
				if user_role_band and user_role_band in vip_list:
					...
					print("VIP LOCATION INNER.........{}".format(location))
					assigned_group = get_assigned_group(location,"vip",category)
					push_ticket_details_mongo(sessionId,Assigned_Group=assigned_group,Urgency="2-High",Impact="2-Significant/Large")
				...
				if chat_stage == "1":
					if str(location_input) not in location_list:
						print("REENTER LOCATION========{}".format(asset_Empty))
                                                user_details_row = userDetailsCollection.find({"cont_id":cont_id}).sort( [{ "_id",-1 }] ).limit(1)
                                                if user_details_row:
                                                        mongoObjjson = json.dumps(list(user_details_row),default=json_util.default)
                                                        mongoObjjson = json.loads(mongoObjjson)
                                                        ...
                                                        if mongoObjjson:
                                                                asset_Empty = mongoObjjson[0].get('asset_Empty')
                                                if asset_Empty == "":
                                                        print("assetStage============{}".format(asset_Empty))
                                                        assetEmpty="true"
                                                        print("chatStage============{}, {}".format(chat_stage, asset_Empty))
                                                        location = input_statement
                                                	...
                                                	location = "asset_Empty"
                                                	chat_stage = "ticket_confirmation"
					else:
                                        	location = "asset_details"
						print("chat stage inner.................{}".format(chat_stage))
						chat_stage = "ticket_confirmation"
                                        	print("chat stage inner.................{}".format(chat_stage))
                                else:
                                        print("chat stage.................{}".format(chat_stage))
                                        note_flag = True
                                        chat_stage = "4"
                                        chat_type = "mobile"
                                        values = {"skip":"Skip"}
		...
		if "kanhe" in location.lower():
			location_inp = input_statement
			print("LOCATION INPUT KANHE..........{}".format(location_inp))
			note_flag = True
			chat_stage = "1i"
			chat_type = "radio"
			...
			asset_Empty = "false"
                        pushUserDetailsToMongo(sessionId, asset_Empty = asset_Empty)
			push_ticket_details_mongo(sessionId,location_inp = "kanhe1")
		elif chat_stage == "ticket_confirmation" or chat_stage == "asset_details":
                        #print "IN ASSET DETAILS======================={}".format(input_statement)
			...
			ticket_details_row = ticket_details_collection.find({"sessionId":sessionId}).sort( [{ "_id",-1 }] ).limit(1)
                	if ticket_details_row:
                        	...
                        	mongoObjjson = filter(None,mongoObjjson)
                        	if mongoObjjson:
                                	location_inp = mongoObjjson[0].get('location_inp')
                	print("LOCATIOPN KAAAAAY AAAHE..............{}".format(location_inp))
			if not location_inp:	
				print("Santoshw as here")
				asset_dict = get_asset_by_user(cont_id)
                       		replyVal = "Okay.</br>Please, select an asset from below for which you're trying to raise a ticket."
                       		values = asset_dict
                       		chat_type = "dropdown"
                     		chat_stage = "2"
                       		replyJson = {"reply":replyVal}
                       		...
                       		if values:
                       		        replyJson["data"]=values
                       		chat_enable = chatEnable(chat_type,force_enable_flag=force_enable_flag)
                       		replyJson.update({"chatEnable":"false"})
                       		replyJson.update({"suggestion_enable":"False"})
                       		replyJson.update({"session_ended":session_ended})
                       		convoTime = datetime.datetime.now()
				asset_Empty = "false"
		...
		ticket_details_row = ticket_details_collection.find({"sessionId":sessionId}).sort( [{ "_id",-1 }] ).limit(1)
        	if ticket_details_row:
                	...
                	if mongoObjjson:
				location_inp = mongoObjjson[0].get('location_inp')
		print("LOCATIOPN KAAAAAY AAAHE..............{}, {}".format(location_inp, input_statement))
		location = input_statement
		if location_inp == "kanhe1" and ("kanhe" in input_statement):
                        if ("kanhe" in input_statement):
				chat_stage = "ticket_confirmation"
				chat_type = "radio"
                        	values = od()
                        	values["confirm"] = "Confirm"
                        	values["cancel"] = "Cancel"
                        	replyVal = respData["id-1008"]["answer"]
				#print "INSIDE KANHE....................{}".format(location.lower())
                        #else:
				asset_dict = get_asset_by_user(cont_id)
                        	replyVal = "Okay.</br>Please, select an asset from below for which you're trying to raise a ticket."
                        	values = asset_dict
			...
			if ("kanhe" not in input_statement):
                                chat_stage = "ticket_confirmation"
                                chat_type = "radio"
                                values = od()
                                values["confirm"] = "Confirm"
		...
		if len(mobile) >= 10:
                ...
                replyJson = {"reply":replyVal}
		#replyVal = getRandomApology()+"<br>"+respData["id-1025"]["answer"].replace("xyz-mobile",mobile)
		...
		mobile = getPhoneNumber(input_statement)
                print("INput STringdfsbfdsd======={}".format(input_statement))
                if getPhoneNumber(input_statement) or input_statement == "skip":
                        wrong_number = "false"
			mobile = input_statement
                        replyVal = respData["id-1012"]["answer"].replace("xyz-loc",company_address)
                else:
		...
		if input_statement == "cancel":
			...
			try:
				print("This is BMC MSOLVE...........{}".format(input_statement))
				ticket_number = create_ticket_bmc(Corporate_ID=Corporate_ID,Summary=Summary, Urgency=Urgency,Impact=Impact,Assigned_Group=Assigned_Group,MNM_Chatbot_ID=sessionId,Notes=Notes,asset=asset)
			...
			push_to_ticket_archieve_mongo(sessionId, incidentId = ticket_number,Corporate_ID=Corporate_ID,Summary=Summary, Urgency=Urgency,Impact=Impact,Assigned_Group=Assigned_Group,MNM_Chatbot_ID=sessionId,Notes=Notes,asset=asset)
...
def application(environ, start_response):
	...
	try:
		...
		if raw_request_json.get("asset"):
                        asset_Empty = raw_request_json.get("asset")
		else:
			asset_Empty = ""
		...
		if d.get('asset',[''])[0]:
                        asset_Empty = d.get('asset',[''])[0]
		else:
                        asset_Empty = ""
	
	pushUserDetailsToMongo(id, asset_Empty = asset_Empty)
