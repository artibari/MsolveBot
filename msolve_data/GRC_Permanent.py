...
def getResponse(input_statement,sessionId,response,cont_id):
	...
	if chat_stage == "sop":
	...
	elif chat_stage == "GRC_request":
	    #Notes = input_statement
		#GRC_file ='/var/www/html/Common Subjects for issues for MBot.csv' 
		#with opne(GRC_file) as grc_f:
		#    grc_issue = csv.reader(grc_f)
		#    firstline = True
		#    for i in grc_issue:
        #        if firstline:
        #            firstline = False 
        ...
        #    	    #print(i[0])
		#			#return issue
		#			print "GRC issues are {}".format(issue)
		replyVal = "Above issue is related to GRC."
		replyJson = {"reply":replyVal}
		chat_type = "radio"
		replyJson.update({"chat_type":chat_type})
	...
	if chat_stage == "2" or chat_stage == "ticket_confirmation":
		...
		if input_statement == "cancel":
			...
			session_ended = "true"
					...
					#push_to_ticket_archieve_mongo(sessionId, Assigned_Group=assigned_group)
