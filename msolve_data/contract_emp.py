...
def handleSingleWords(answerId):
    with open('/var/www/html/mSolvebot_response.json') as resp_file:
        respDate =json.load(resp_file)
    reply_string =""
    if answerId =='id-129':
        reply_string = respData['id-1005']['answer']
    return reply_string

def get_assigned_group(user_location):

    group_join = '/var/www/html/cont_emp_location.csv'
    #print "User input is {}".format(user_location)
    user_location = user_location.lower()
    print "User Input is  .........{}".format(user_location)
    asset_group = get_asset_cont_emp()
    for workgroup,loc in asset_group.items():
        # workgroup = workgroup.lower()
        # print "Work group is ...==============={}".format(workgroup)
         if workgroup.lower() == user_location:
             print "WorkGroup for selected location is ....{}".format(loc.lower())
             #return workgroup.lower()
             return loc.lower()
         else:
             continue 
...
def getResponse(input_statement,sessionId,response,cont_id):
    ...
    ##response ="/var/www/html/mSolvebot_response.json"
    ##resp_file = open("/var/www/html/mSolvebot_response.json")
    with open("/var/www/html/mSolvebot_response.json","r") as resp_file:
	    respData = json.load(resp_file)
            ##response = respData["id-1015"]["answer"]
            ##print response
            ##print respData
    ...
    if input_statement =='createit':
       ...
       # print ticket_list
        ...
        replyJson.update({"chatEnable":chat_enable})
        replyJson.update({"chat_type":"card"})
        replyJson.update({"session_ended":session_ended})
        convoTime = datetime.datetime.now()
        if user == "MSolve":
        ...
        #print "JSON DATA +++++++++++++++++{}".format(replyJson)
        return replyJson
        ...
        ##print "Your inside the IncideentReq" 
        ##print replyJson
        return replyJson
    ...
    if chat_stage == "sop":
        ...
        if len(input_statement.split()) < 2:
            ...
            resp = requests.request("get","https://emss.mahindra.com/sap/bc/zzhr_emp_details?CONT-ID="+Corporate_ID)
        ...
        if len(input_statement.split()) < 2:
            ...
            resp = requests.request("get","https://emss.mahindra.com/sap/bc/zzhr_emp_details?CONT-ID="+Corporate_ID)
            resp_list = resp.text.split("-//-")
            print "Inside the service request {}".format(resp_list)
            try:
                mobile = resp_list[18]
                print mobile 
                pushUserDetailsToMongo(sessionId, mobile=mobile)
       ...
       asset_stage = get_asset_cont_emp()
       #print "Asset dict is .......{}".format(asset_stage)
       replyVal = "Workgroup for selected location is {}".format(input_statement)
       replyJson = {"reply":replyVal}
       chat_type = "radio"
       note_flag = False
       location =[]
       loca = "/var/www/html/cont_emp_location.csv" 
       with open(loca) as emp_loc:
           cont_loc = csv.reader(emp_loc)
           firstline = True
           for i in cont_loc:
               if firstline:
                   firstline = False
                   ...
                   #force_enable_flag = "on"
       ##if str(input_statement.lower()) in location:
       group = get_assigned_group(input_statement) 
       push_ticket_details_mongo(sessionId, Assigned_Group =assigned_group )
       #print "Assigned group is .....{}".format(Assigned_Group)
       #print "User selected location is ***********{}".format(group)
       ...
       pushLastResponseInMongo(sessionId,replyVal,category,sub_bucket,input_statement,count_reply,mongo_bucket,note_flag,chat_stage,ticket_number,session_ended, ticket_type)
       ##print "Ticket Number is .=============>>>>>>>>{}".format(ticket_number)      
       ...
       # return replyJson
         ...
         if input_statement.lower() == "skip":
                 ...
                 replyVal = respData["id-1010"]["answer"]+"<br>"+respData["id-1013"]["answer"]
                 chat_stage = "msolve_address_incidentreq"
                 values = {"skip":"Skip"}
                 force_enable_flag = "on"
                 chat_type = "address"
                 ...
                 data = '/var/www/html/cont_emp_location.csv'
                 location = []
                 with open(data) as contEmp:
                      contLoc = csv.reader(contEmp)
                      firstline = True
                      for loc in contLoc:
                          if firstline:
                              firstline = False
                 ...
                 replyVal = "Please Enter the correct Mobile Number"
                 #replyVal = "Selected Location is {}".format(input_statement.upper())
                 chat_stage = "msolve_address_incidentreq"
    ...
    if chat_stage == "mobile_incidentreq":
         ...
         if input_statement.lower() == "skip":
                 ...
                 #replyVal = "Please select your location from below list............"
                 replyVal = "Selected Location is {}".format(input_statement.upper()) 
                 chat_stage = "msolve_address_incidentreq"
         replyJson = {"reply":replyVal}
         replyJson.update({"chat_type":chat_type})
         ...
         pushLastResponseInMongo(sessionId,replyVal,category,sub_bucket,input_statement,count_reply,mongo_bucket,note_flag,chat_stage,ticket_number,session_ended, ticket_type)
         pushConversationLogsToMongo(sessionId,convoTime,input_statement,replyVal,user_name,cont_id,Notes)
         ## print "Reply is ........{}".format(replyJson)
    ...
    if chat_stage == "checkstatus":
        if input_statement.upper().startswith("INC") and len(input_statement) == 15 or input_statement.upper().startswith("WO") and len(input_statement) == 15:
            ...
            try:
                ...
                print "Ticket state is ****************{}".format(ticket_stat)
			...
			# ticket_stat,comment = status_ticket_bmc(input_statement.upper())
                ...
                replyJson = {"reply":replyVal}
                print "TTTTTTTTTicket SSSSStatus ***********{}".format(replyJson)
				# chat_type = "radio"
                chat_type = "phone"
                #chat_type = "card"
                replyJson.update({"chat_type":"phone"})
                replyJson.update({"chat_unable":chat_enable})
        ...
        if input_statement == "cancel":
            ...
            if ticket_type == "incident":
                try:
                    ...
                    print "Corporate Id ++++++++=======+++++++++{}".format(Corporate_ID)
                   # print "Mob numb ======================{}".format(mobile)
                   # print "token number===={}".format(user)
                   # print "Issue {}".format(Summary)
                   # print "------------{}".format(Notes)
                   # print "------------{}".format(asset)
                   # print "----------------{}".format(sessionId)
                   # print "------------------{}".format(Assigned_Group)
                    ...
                    #print "Ticket Number +++++++++++++++{}".format(ticket_number)
                except:
          ...
          ##print "Ticekt Number is ==============={}".format(ticket_number)
...
def application(environ, start_response):
	...
	if last_resp_row:
                print "Before .........{}".format(last_resp_row)
		mongoObjjson = json.dumps(list(last_resp_row),default=json_util.default)
                print "After ..........{}".format(mongoObjjson)
		mongoObjjson = json.loads(mongoObjjson)
                #print "After ..........{}".format(last_resp_row)
	        ##print "Mongo Objcet {}".format(mongoObjjson)
                mongoObjjson = filter(None,mongoObjjson)
		if mongoObjjson:
	...
	jsonReply = json.dumps(reply)
        reply["reply"]= "Alright.</br>Please elaborate your issue.</br>So that our support team can help you better."
